...
class SessionStore {
        ...
        if (raw) {
          ...
          this.userStories = parsed.filter((s) => {
            const expiresAt = s.expires_at
              ...
              : new Date(s.created_at).getTime() + 86400000;
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        window.localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify(this.userStories),
        );
      }
    } catch (e) {
  ...
  public addStory(story: Story) {
    ...
    this.saveToStorage();
    console.log(
      `[SessionStore] Added 24h story: ${story.id}. Total cached stories: ${this.userStories.length}`,
    );
  }
