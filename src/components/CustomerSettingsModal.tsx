...
export const CustomerSettingsModal: React.FC<CustomerSettingsModalProps> = ({
  ...
  const [activeSubscriptions, setActiveSubscriptions] = useState([
    { id: 'sub-1', creator_name: 'Alex Vance', tier: 'Gold VIP', price: '$9.99/mo', renew_date: '2026-08-20' },
    { id: 'sub-2', creator_name: 'Sarah Connor', tier: 'Platinum VIP', price: '$19.99/mo', renew_date: '2026-08-15' },
  ]);
  const [unlockedSnaps, setUnlockedSnaps] = useState([
    { id: 'snap-1', title: 'Exclusive VIP Beach Snap', price: '$1.99', date: '2026-07-20' },
    { id: 'snap-2', title: 'Behind The Scenes Reel', price: '$2.99', date: '2026-07-18' },
  ]);
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        if (e.target?.result) {
          ...
          if (typeof window !== 'undefined') window.alert('📸 Profile picture updated!');
        }
      };
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.profileCard}>
              ...
              <View style={styles.profileDetails}>
                ...
                <Text style={styles.badgeText}>👤 Customer / Subscriber Account</Text>
              </View>
              {onSignOut && (
            ...
            <View style={styles.photoActionRow}>
              <TouchableOpacity style={styles.uploadBtn} onPress={handleDevicePhotoUpload}>
            ...
            <View style={styles.summaryCard}>
              ...
              <Text style={styles.summaryDesc}>
                Total spent on VIP Creator Subscriptions and Unlocked Premium Pay-Per-View Snaps.
              </Text>
            </View>

            {/* ACTIVE SUBSCRIPTIONS */}
            <View style={styles.sectionCard}>
              <Text style={styles.sectionTitle}>👑 Active VIP Subscriptions ({activeSubscriptions.length})</Text>
              {activeSubscriptions.map((sub) => (
                <View key={sub.id} style={styles.itemRow}>
                  <View>
                    <Text style={styles.itemTitle}>{sub.creator_name}</Text>
                    <Text style={styles.itemSubtitle}>Tier: {sub.tier} ({sub.price})</Text>
                  </View>
            ...
            {/* UNLOCKED PPV SNAPS */}
            <View style={styles.sectionCard}>
              <Text style={styles.sectionTitle}>🔒 Unlocked Premium Snaps ({unlockedSnaps.length})</Text>
              {unlockedSnaps.map((snap) => (
                <View key={snap.id} style={styles.itemRow}>
                  <View>
                    <Text style={styles.itemTitle}>{snap.title}</Text>
                    <Text style={styles.itemSubtitle}>Unlocked on {snap.date}</Text>
                  </View>
            ...
            <View style={styles.upgradeCard}>
              <Text style={styles.upgradeTitle}>🎨 Want to Monetize Your Own Snaps?</Text>
              <Text style={styles.upgradeDesc}>
                Become a Content Creator today to sell VIP Memberships and Pay-Per-View Snaps with direct 95% Stripe payouts!
              </Text>
              <TouchableOpacity
              ...
              >
                <Text style={styles.upgradeBtnText}>🚀 Upgrade to Creator Account</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        )}
