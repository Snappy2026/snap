...
let useCameraPermissionHook: any = () => ({
  hasPermission: true,
  requestPermission: async () => true,
});
let useMicrophonePermissionHook: any = () => ({
  hasPermission: true,
  requestPermission: async () => true,
});
...
export const CameraScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (
      ...
      navigator.mediaDevices
        ...
        .catch((err) => {
          console.warn(
            "[Web Camera Warning] Webcam permission or device notice:",
            err.message,
          );
    ...
    try {
      ...
      if (Platform.OS === "web") {
        if (webVideoRef.current) {
          ...
          const dummySvg =
            'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="600" height="800"><rect width="100%" height="100%" fill="%231a1a2e"/><text x="50%" y="50%" fill="%23FFFC00" font-size="30" font-family="sans-serif" text-anchor="middle">Adult+ Web Snap</text></svg>';
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Photo capture failed";
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        cameraRef.current.startRecording({
          ...
          onRecordingError: (error: any) => {
            ...
            recordingProgress.value = 0;
            console.error(
              "[Camera Error] Video recording failed:",
              error.message,
            );
          },
        });
      ...
      setIsRecording(false);
      const errorMessage =
        err instanceof Error ? err.message : "Recording error";
  ...
  const stopRecording = async () => {
    ...
    if (Platform.OS === "web") {
      ...
      if (
        ...
        const dummySvg =
          'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="600" height="800"><rect width="100%" height="100%" fill="%2316213e"/><text x="50%" y="50%" fill="%23FF3B30" font-size="30" font-family="sans-serif" text-anchor="middle">Adult+ Web Video Snap</text></svg>';
  ...
  const processAndNavigateToSendTo = (
    fileUri: string,
    type: "image" | "video",
  ) => {
  ...
  const handleSaveCapturedMedia = () => {
    ...
    try {
      if (Platform.OS === "web" && typeof document !== "undefined") {
        ...
        a.download = `adultplus-capture-${Date.now()}.${capturedMedia.type === "video" ? "webm" : "jpg"}`;
        ...
        Alert.alert(
          "💾 Snap Saved!",
          "Photo saved to your device camera roll.",
        );
      }
    } catch (e) {
  ...
  const shutterGesture = Gesture.Simultaneous(
    singleTapGesture,
    longPressGesture,
  );
      ...
      <View style={styles.container}>
          ...
          <View style={styles.webCameraContainer}>
            ...
            {!webStreamActive && (
              <View style={styles.webFallbackOverlay}>
                ...
                <Text style={styles.webCameraTitle}>Adult+ Camera View</Text>
        ...
        <View style={styles.sideControls}>
          <TouchableOpacity
            style={[
              styles.iconButton,
              flashMode === "on" && styles.iconButtonFlashActive,
            ]}
        ...
        <View style={styles.bottomControls}>
          {!isProcessing && (
            <View style={styles.shutterRow}>
              <GestureDetector
                gesture={Gesture.Race(singleTapGesture, longPressGesture)}
              >
                <Animated.View
                  style={[styles.shutterOuterRing, animatedShutterStyle]}
                >
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={takePhoto}
        ...
        {capturedMedia && (
          <View style={styles.capturedOverlay}>
            ...
            <SafeAreaView style={styles.capturedTopBar}>
              ...
              <TouchableOpacity
                style={styles.saveHeaderIconBtn}
                onPress={handleSaveCapturedMedia}
              >
            ...
            <View style={styles.capturedBottomBar}>
              <TouchableOpacity
                style={styles.storyPostBtn}
                onPress={handlePostCapturedMediaToStory}
              >
                <Text style={styles.storyPostText}>👻 My Story</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={styles.vipPostBtn}
                onPress={handlePostCapturedMediaToVipStory}
              >
                <Text style={styles.vipPostText}>👑 VIP Story</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={styles.sendToFabBtn}
                onPress={handleOpenSendToModal}
              >
