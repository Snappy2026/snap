...
export const SendToModal: React.FC = () => {
  ...
  const handleSend = async () => {
    if (!postToStory && selectedRecipientIds.length === 0) {
      Alert.alert(
        "Select Recipient",
        "Please select at least one friend or My Story.",
      );
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to send snap";
  ...
  const handleSaveToDevice = () => {
    try {
      if (Platform.OS === "web" && typeof document !== "undefined") {
        ...
        a.href = mediaUrl;
        a.download = `adultplus-capture-${Date.now()}.${mediaType === "video" ? "webm" : "jpg"}`;
      ...
      <View style={styles.previewCard}>
        ...
        <View style={styles.previewInfo}>
          <Text style={styles.previewTitle}>
            Captured {mediaType === "video" ? "Video" : "Photo"} Snap
          </Text>
      ...
      <View style={styles.ppvConfigCard}>
        <View style={styles.ppvHeaderRow}>
          ...
          >
            <Text
              style={[
                styles.ppvOptionText,
                !isPpv && styles.ppvOptionTextActive,
              ]}
            >
              🔓 Free Snap
            </Text>
        ...
        {isPpv && (
          <View style={styles.ppvPriceRow}>
            ...
            <Text style={styles.ppvFeeNote}>
              5% Admin Fee: ${(parseFloat(ppvPrice || "0") * 0.05).toFixed(2)} |
              Net to you: ${(parseFloat(ppvPrice || "0") * 0.95).toFixed(2)}
            </Text>
          </View>
        )}
      </View>
      ...
      >
        ...
        <View style={styles.storyDetails}>
          ...
          <Text style={styles.storySubtitle}>
            Visible to all friends for 24 hours
          </Text>
        </View>
